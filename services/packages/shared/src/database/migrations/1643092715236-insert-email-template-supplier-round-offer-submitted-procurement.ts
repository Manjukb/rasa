...
export class insertEmailTemplateSupplierRoundOfferSubmittedProcurement1643092715236 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        const checkTemplate = await queryRunner.manager.findOne(EmailTemplate, {
            template_type: TemplateType.SupplierRoundOfferSubmittedProcurement,
        });

        const template = new EmailTemplate();
        template.id = '7df95be93a95417d878f2a52280858ac';
        template.organisation_id = null;
        template.template_type = TemplateType.SupplierRoundOfferSubmittedProcurement;
        template.subject = 'Supplier Offer Submission for RFQ RefNo:{{rfq_number}}';
        template.message = `<!DOCTYPE html>
        <html lang="en">
          <head>
            <title>SupplierRoundOfferSubmittedProcurement</title>
            ...
            <div role="article" lang="en" style="background-color: #167d7f; color: #2b2b2b; font-family: 'Avenir Next', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; font-size: 18px; font-weight: 400; line-height: 28px; margin: 0 auto; max-width: 600px; padding: 40px 20px 40px 20px;">
                ...
                <main>
                    <div style="background-color: ghostwhite; border-radius: 4px; padding: 24px 48px;">
                        ...
                        <p>
                            Supplier {{supplier_name}} from {{supplier_organisation}} has submitted their Round {{negotiation_round_no}} response on {{supplier_offer_submit_date}}. You can view the latest status of the negotiation from this link: <a href="{{link_to_rfq_negotiation_detail_table}}"><button style="background-color: #29a0b1;
                                color: white;
                                text-align: center;

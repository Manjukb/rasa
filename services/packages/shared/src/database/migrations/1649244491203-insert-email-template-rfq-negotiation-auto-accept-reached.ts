...
export class insertEmailTemplateRfqNegotiationAutoAcceptReached1649244491203 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        const checkTemplate = await queryRunner.manager.findOne(EmailTemplate, {
            template_type: TemplateType.AutoAcceptanceScoreReached,
        });

        const template = new EmailTemplate();
        template.id = '1f4a3c2705b44469bba9fd514063aaf4';
        template.organisation_id = null;
        template.template_type = TemplateType.AutoAcceptanceScoreReached;
        template.subject = 'The Negotiation Process for RFQ RefNo:{{rfq_number}} is completed';
        template.message = `<!DOCTYPE html>
        <html lang="en">
          <head>
            <title>AutoAcceptanceScoreReached</title>
            ...
            <div role="article" lang="en" style="background-color: #167d7f; color: #2b2b2b; font-family: 'Avenir Next', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; font-size: 18px; font-weight: 400; line-height: 28px; margin: 0 auto; max-width: 600px; padding: 40px 20px 40px 20px;">
                ...
                <main>
                    <div style="background-color: ghostwhite; border-radius: 4px; padding: 24px 48px;">
                        ...
                        <p>
                            The negotiation process has been completed for RFQ Reference No: {{rfq_number}} as an auto-accept score has been reached. You can view the latest status of the negotiation from this link: <a href="{{link_to_rfq_negotiation_detail_table}}"><button style="background-color: #29a0b1;
                                color: white;
                                text-align: center;
